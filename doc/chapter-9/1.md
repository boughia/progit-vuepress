...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> svnsync init file:///tmp/test-svn <span style="font-weight: bold; font-style: italic">\</span>
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git svn clone file:///tmp/test-svn -s</code></pre>
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> tree .git/refs
.git/refs
├── heads
│   └── master
├── hg
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> cp .hgignore .git/info/exclude</code></pre>
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git push
To hg::/tmp/hello
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git checkout -b featureA
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> hg branches
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> wget https://raw.github.com/felipec/git-remote-bzr/master/git-remote-bzr -O ~/bin/git-remote-bzr
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> export GIT_SSL_NO_VERIFY=true</code></pre>
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> cd myproject
...
<pre class="language-bash"><code><span style="font-weight: bold">#</span> A Perforce Change Specification.
...
<p>Git-tf（主页在 <a href="https://archive.codeplex.com/?p=gittf" class="link">https://archive.codeplex.com/?p=gittf</a>）是一个 Java 项目，
...
Codeplex (<a href="https://archive.codeplex.com/" class="link">https://archive.codeplex.com/</a>) 或 Visual Studio Online (<a href="http://www.visualstudio.com" class="link">http://www.visualstudio.com</a>) 都是非常好的选择。</p>
</div>
</aside>
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git tf clone https://tfs.codeplex.com:443/tfs/TFS13 $/myproject/Main project_git</code></pre>
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> cd project_git
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git tf clone https://tfs.codeplex.com:443/tfs/TFS13 $/myproject/Main <span style="font-weight: bold; font-style: italic">\</span>
  project_git --deep
...
<pre class="source language-powershell"><code>PS&gt; git tfs clone --with-branches \
...
<pre class="source language-powershell"><code>PS&gt; git log --oneline --graph --decorate --all
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git config set --local core.ignorecase=true
...
<ol class="arabic">
<li>
<span class="principal">TFVC 无法表示主题分支，这会增加一点复杂度。
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git log --oneline --graph --decorate --all
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git tf fetch
Username: domain\user
Password:
Connecting to TFS...
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git rebase FETCH_HEAD
First, rewinding head to replay your work on top of it...
...
<pre class="language-bash"><code><span style="font-weight: bold">$</span> git tf checkin -m <span style="font-style: italic">&#39;Updating readme and code&#39;</span>
Username: domain\user
Password:
Connecting to TFS...
...
<pre class="source language-powershell"><code>PS&gt; git log --oneline --graph --all --decorate
...
<pre class="source language-powershell"><code>PS&gt; git tfs fetch
...
<pre class="source language-powershell"><code>PS&gt; git rebase tfs/<span style="font-weight: bold">default</span>
First, rewinding head to replay your work on top of it...
...
<pre class="source language-powershell"><code>PS&gt; git tfs rcheckin
...
<pre class="source language-powershell"><code>PS&gt; git tfs checkintool
...
<pre class="source language-powershell"><code>PS&gt; git tfs branch $/tfvc-test/featureBee
...
如果你想要在新生成的分支上工作，那你也许应该通过从那次提交创建一个主题分支的方式使你新的提交基于 <code class="literal">1d54865</code> 提交。</p>
